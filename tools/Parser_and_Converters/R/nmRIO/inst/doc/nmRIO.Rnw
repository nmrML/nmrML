%\VignetteIndexEntry{nmRIO, Ramp, mzXML, mzData, mzML}
...
The \Rpackage{nmRIO} \citep{Chambers2012} package aims at providing a common interface to
...
done in nmRIO, which is left to packages such as 
...
The \Rpackage{nmRIO} package is in essence a collection of wrappers to the C++
...
%% Where possible \Rpackage{nmRIO} selects the appropriate backend for the 
