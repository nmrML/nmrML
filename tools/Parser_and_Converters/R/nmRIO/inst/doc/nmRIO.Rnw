%\VignetteIndexEntry{nmRIO}
%\VignetteKeywords{Infrastructure, DataImport, Metabolomics, NMR}
...
\title{A Parser for \texttt{nmrML} files}

\author{Jie Hao\footnote{\email{j.hao@imperial.ac.uk}} \\
  Ian Lewis\footnote{\email{ilewis@Princeton.EDU}} \\
...
several NMR data formats, namely \texttt{nmrML}, modelled after the
PSI mzML~\cite{Martens2010}.
