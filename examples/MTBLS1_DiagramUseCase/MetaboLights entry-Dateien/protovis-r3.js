...
if (!Array.prototype.map)
    Array.prototype.map = function(b, c) {
        for (var d = this.length, f = new Array(d), g = 0; g < d; g++)
            if (g in this)
                f[g] = b.call(c, this[g], g, this);
        return f
    };
if (!Array.prototype.filter)
    Array.prototype.filter = function(b, c) {
    ...
    c.parse = function(f) {
        var g = 1970, h = 0, i = 1, j = 0, l = 0, k = 0, q = [function() {
            ...
            switch (n) {
                case "%b":
                    q.push(function(m) {
                        h = {Jan: 0,Feb: 1,Mar: 2,Apr: 3,May: 4,Jun: 5,Jul: 6,Aug: 7,Sep: 8,Oct: 9,Nov: 10,Dec: 11}[m]
                    });
                    return "([A-Za-z]+)";
                case "%h":
                case "%B":
                    q.push(function(m) {
                        h = {January: 0,February: 1,March: 2,April: 3,May: 4,June: 5,July: 6,August: 7,September: 8,October: 9,November: 10,December: 11}[m]
                    });
                    return "([A-Za-z]+)";
                case "%e":
                case "%d":
...
pv.Format.time = function(b) {
    function c(f) {
        ...
        switch (b) {
            case "short":
                if (f >= 31536E6)
                    return (f / 31536E6).toFixed(1) + " years";
                else if (f >= 6048E5)
                    return (f / 6048E5).toFixed(1) + " weeks";
                else if (f >= 864E5)
                    return (f / 864E5).toFixed(1) + " days";
                else if (f >= 36E5)
                    return (f / 36E5).toFixed(1) + " hours";
                else if (f >= 6E4)
                    return (f / 6E4).toFixed(1) + " minutes";
    ...
    c.parse = function(f) {
        switch (b) {
            ...
            case "long":
                ...
                i = 0;
                if (h.length)
                    i += parseFloat(h[0]) * 1E3;
                if (h.length > 1)
                    i += parseFloat(h[1]) * 6E4;
                if (h.length > 2)
                    i += parseFloat(h[2]) * 36E5;
                if (h.length > 3)
                    i += parseFloat(h[3]) * 864E5;
                return i
        }
    };
    return c
...
pv.Format.number = function() {
    function b(n) {
        ...
        n = n < 0 ? "-" : "";
        if (r.length > d)
            r = r.substring(r.length - d);
        if (k && r.length < c)
            r = n + (new Array(c - r.length + 1)).join(j) + r;
        if (r.length > 3)
            r = r.replace(/\B(?=(?:\d{3})+(?!\d))/g, o);
        if (!k && r.length < f)
            r = (new Array(f - r.length + 1)).join(j) + n + r;
        m[0] = r;
        r = m[1] || "";
        if (r.length < g)
            m[1] = r + (new Array(g - r.length + 1)).join(l);
        return m.join(q)
    }
...
pv.cross = function(b, c) {
    for (var d = [], f = 0, g = b.length, h = c.length; f < g; f++)
        for (var i = 0, j = b[f]; i < h; i++)
            d.push([j, c[i]]);
    return d
};
pv.blend = function(b) {
...
a.removeChild = function(b) {
    ...
    if (b.previousSibling)
        ...
        this.firstChild = b.nextSibling;
    if (b.nextSibling)
        b.nextSibling.previousSibling = b.previousSibling;
    else
        this.lastChild = b.previousSibling;
...
a.appendChild = function(b) {
    ...
    b.parentNode = this;
    if (b.previousSibling = this.lastChild)
        this.lastChild.nextSibling = b;
    else
        this.firstChild = b;
    this.lastChild = b;
...
a.insertBefore = function(b, c) {
    ...
    b.parentNode = this;
    b.nextSibling = c;
    if (b.previousSibling = c.previousSibling)
        c.previousSibling.nextSibling = b;
    else {
        if (c == this.lastChild)
            this.lastChild = b;
        this.firstChild = b
...
a.replaceChild = function(b, c) {
    ...
    if (b.previousSibling = c.previousSibling)
        ...
        this.firstChild = b;
    if (c.nextSibling)
        c.nextSibling.previousSibling = b;
    else
        this.lastChild = b;
    this.childNodes[d] = b;
    return c
...
a.reverse = function() {
    ...
    this.visitAfter(function(c) {
        for (; c.lastChild; )
            b.push(c.removeChild(c.lastChild));
        for (var d; d = b.pop(); )
...
a.toggle = function(b) {
    if (b)
        return this.toggled ? this.visitBefore(function(d) {
            d.toggled && d.toggle()
    ...
    b = this;
    if (b.toggled) {
        for (var c; c = b.toggled.pop(); )
            b.appendChild(c);
        delete b.toggled
    } else if (b.lastChild)
        for (b.toggled = []; b.lastChild; )
            b.toggled.push(b.removeChild(b.lastChild))
};
pv.nodes = function(b) {
    for (var c = new pv.Dom.Node, d = 0; d < b.length; d++)
        c.appendChild(new pv.Dom.Node(b[d]));
    return c.nodes()
};
pv.tree = function(b) {
...
pv.Tree.prototype.map = function() {
    for (var b = {}, c = {}, d = 0; d < this.array.length; d++) {
        c.index = d;
        for (var f = this.array[d], g = this.k.call(c, f), h = b, i = 0; i < g.length - 1; i++)
            h = h[g[i]] || (h[g[i]] = {});
...
pv.Scale.quantitative = function() {
    ...
    c.range = function() {
        if (arguments.length) {
            ...
            1; o++)
                h.push(pv.Scale.interpolator(g[o], g[o + 1]));
            return this
        }
        return g
    };
...
pv.SvgScene.updateAll = function(b) {
    if (b.length && b[0].reverse && b.type != "line" && b.type != "area") {
        for (var c = pv.extend(b), d = 0, f = b.length - 1; f >= 0; d++, f--)
            c[d] = b[f];
        b = c
    }
...
pv.SvgScene.expect = function(b, c, d, f) {
    ...
    for (var g in d) {
        c = d[g];
        if (c == this.implicit.svg[g])
            c = null;
        c == null ? b.removeAttribute(g) : b.setAttribute(g, c)
    }
    for (g in f) {
        c = f[g];
        if (c == this.implicit.css[g])
            c = null;
        if (c == null)
...
pv.SvgScene.pathBasis = function() {
    function b(f, g, h, i, j) {
        return {x: f[0] * g.left + f[1] * h.left + f[2] * i.left + f[3] * j.left,y: f[0] * g.top + f[1] * h.top + f[2] * i.top + f[3] * j.top}
    }
...
pv.SvgScene.curveHermite = function(b, c) {
    if (c.length < 1 || b.length != c.length && b.length != c.length + 2)
        return "";
...
pv.SvgScene.monotoneTangents = function(b) {
    ...
    for (h = 0; h < b.length - 1; h++)
        d[h] = (b[h + 1].top - b[h].top) / (b[h + 1].left - b[h].left);
    f[0] = d[0];
    ...
    for (h = 0; h < b.length - 1; h++)
        if (!(Math.abs(f[h]) < 1.0E-5 || Math.abs(f[h + 1]) < 1.0E-5)) {
...
pv.SvgScene.area = function(b) {
    ...
    var g = b[0];
    if (g.segmented)
        return this.areaSegment(b);
    if (!g.visible)
        return f;
    var h = g.fillStyle, i = g.strokeStyle;
    if (!h.opacity && !i.opacity)
        return f;
    for (var j = 
    [], l, k = 0; k < b.length; k++) {
        l = b[k];
    ...
    if (!j.length)
        return f;
    f = this.expect(f, "path", {"shape-rendering": g.antialias ? null : "crispEdges","pointer-events": g.events,cursor: g.cursor,d: "M" + j.join("ZM") + "Z",fill: h.color,"fill-opacity": h.opacity || 
        null,stroke: i.color,"stroke-opacity": i.opacity || null,"stroke-width": i.opacity ? g.lineWidth / this.scale : null});
    ...
    for (i = b.length - 1; h < i; h++) {
        ...
        if (d.visible && l.visible) {
            ...
            if (k.opacity || q.opacity) {
                ...
                c = this.expect(c, "path", {"shape-rendering": d.antialias ? null : 
                    "crispEdges","pointer-events": d.events,cursor: d.cursor,d: j,fill: k.color,"fill-opacity": k.opacity || null,stroke: q.color,"stroke-opacity": q.opacity || null,"stroke-width": q.opacity ? d.lineWidth / this.scale : null});
                c = this.append(c, b, h)
            }
        }
    }
    ...
    for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
        ...
        if (f.visible) {
            ...
            if (g.opacity || h.opacity) {
                ...
                g = {"shape-rendering": f.antialias ? null : "crispEdges","pointer-events": f.events,cursor: f.cursor,fill: g.color,"fill-opacity": g.opacity || null,stroke: h.color,"stroke-opacity": h.opacity || null,"stroke-width": h.opacity ? f.lineWidth / this.scale : null};
                if (j) {
                    g.transform = "translate(" + f.left + "," + f.top + ")";
                    if (f.angle)
                        g.transform += " rotate(" + 180 * f.angle / Math.PI + ")";
                    g.d = j;
...
pv.SvgScene.image = function(b) {
    for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
        ...
        if (f.visible) {
            c = this.fill(c, b, d);
            if (f.image) {
                c = this.expect(c, "foreignObject", {cursor: f.cursor,x: f.left,y: f.top,width: f.width,height: f.height});
                ...
                {preserveAspectRatio: "none",cursor: f.cursor,x: f.left,y: f.top,width: f.width,height: f.height});
    ...
    for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
        ...
        if (f.visible) {
            ...
            if (g.opacity && f.text) {
                ...
                c = this.expect(c, "text", {"pointer-events": f.events,cursor: f.cursor,x: h,
                    y: i,dy: j,transform: "translate(" + f.left + "," + f.top + ")" + (f.textAngle ? " rotate(" + 180 * f.textAngle / Math.PI + ")" : "") + (this.scale != 1 ? " scale(" + 1 / this.scale + ")" : ""),fill: g.color,"fill-opacity": g.opacity || null,"text-anchor": l}, {font: f.font,"text-shadow": f.textShadow,"text-decoration": f.textDecoration});
                if (c.firstChild)
                    c.firstChild.nodeValue = f.text;
                else
                    c.appendChild(document.createTextNode(f.text));
                c = this.append(c, b, d)
            }
        }
    }
...
pv.SvgScene.line = function(b) {
    ...
    var d = b[0];
    if (d.segmented)
        return this.lineSegment(b);
    if (!d.visible)
        return c;
    var f = d.fillStyle, g = d.strokeStyle;
    if (!f.opacity && !g.opacity)
        return c;
    var h = "M" + d.left + "," + d.top;
    if (b.length > 2 && (d.interpolate == "basis" || d.interpolate == "cardinal" || d.interpolate == "monotone"))
        switch (d.interpolate) {
            case "basis":
                h += this.curveBasis(b);
                break;
        ...
        for (var i = 1; i < b.length; i++)
            h += this.pathSegment(b[i - 1], b[i]);
    c = this.expect(c, "path", {"shape-rendering": d.antialias ? null : "crispEdges","pointer-events": d.events,cursor: d.cursor,d: h,fill: f.color,"fill-opacity": f.opacity || null,stroke: g.color,"stroke-opacity": g.opacity || null,"stroke-width": g.opacity ? d.lineWidth / this.scale : null,"stroke-linejoin": d.lineJoin});
    ...
    for (var g = b.length - 1; d < g; d++) {
        ...
        if (h.visible && i.visible) {
            ...
            if (j.opacity) {
                ...
                c = this.expect(c, "path", {"shape-rendering": h.antialias ? null : "crispEdges","pointer-events": h.events,cursor: h.cursor,d: i,fill: l.color,"fill-opacity": l.opacity || null,stroke: j.color,"stroke-opacity": j.opacity || null,"stroke-width": j.opacity ? h.lineWidth / this.scale : null,"stroke-linejoin": h.lineJoin});
                c = this.append(c, b, d)
            }
        }
    }
    ...
    for (var c = b.$g, d = c && c.firstChild, f = 0; f < b.length; f++) {
        ...
        if (g.visible) {
            if (!b.parent) {
                ...
                if (c && c.parentNode != g.canvas)
                    d = (c = g.canvas.firstChild) && c.firstChild;
                if (!c) {
                    ...
                    for (var h = 0; h < this.events.length; h++)
                        c.addEventListener(this.events[h], 
                        this.dispatch, false);
                    d = c.firstChild
                }
                b.$g = c;
...
pv.SvgScene.fill = function(b, c, d) {
    ...
    if (g.opacity || f.events == "all") {
        b = this.expect(b, "rect", {"shape-rendering": f.antialias ? null : "crispEdges","pointer-events": f.events,cursor: f.cursor,x: f.left,y: f.top,width: f.width,height: f.height,fill: g.color,"fill-opacity": g.opacity,stroke: null});
        b = this.append(b, c, d)
    }
    return b
};
pv.SvgScene.stroke = function(b, c, d) {
    ...
    if (g.opacity || f.events == "all") {
        b = this.expect(b, "rect", {"shape-rendering": f.antialias ? null : "crispEdges","pointer-events": f.events == "all" ? "stroke" : f.events,cursor: f.cursor,x: f.left,y: f.top,width: Math.max(1.0E-10, f.width),height: Math.max(1.0E-10, f.height),fill: null,stroke: g.color,"stroke-opacity": g.opacity,"stroke-width": f.lineWidth / this.scale});
        b = this.append(b, c, d)
    }
    return b
};
pv.SvgScene.rule = function(b) {
    for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
        ...
        if (f.visible) {
            var g = f.strokeStyle;
            if (g.opacity) {
                c = this.expect(c, "line", {"shape-rendering": f.antialias ? null : "crispEdges","pointer-events": f.events,cursor: f.cursor,x1: f.left,y1: f.top,x2: f.left + f.width,y2: f.top + f.height,stroke: g.color,"stroke-opacity": g.opacity,"stroke-width": f.lineWidth / this.scale});
                c = this.append(c, b, d)
            }
        }
    }
    ...
    for (var c = b.$g.firstChild, d = 0; d < b.length; d++) {
        ...
        if (f.visible) {
            ...
            if (g.opacity || h.opacity) {
                ...
                if (l >= 2 * Math.PI)
                    i = i ? "M0," + j + "A" + j + "," + j + " 0 1,1 0," + -j + "A" + j + "," + j + " 0 1,1 0," + j + "M0," + i + "A" + i + "," + i + " 0 1,1 0," + -i + "A" + i + "," + i + " 0 1,1 0," + i + "Z" : "M0," + j + "A" + j + "," + j + " 0 1,1 0," + -j + "A" + j + "," + j + " 0 1,1 0," + j + "Z";
                else {
                ...
                c = this.expect(c, "path", {"shape-rendering": f.antialias ? null : "crispEdges","pointer-events": f.events,cursor: f.cursor,transform: "translate(" + f.left + "," + f.top + ")",d: i,fill: g.color,"fill-rule": "evenodd","fill-opacity": g.opacity || 
                    null,stroke: h.color,"stroke-opacity": h.opacity || null,"stroke-width": h.opacity ? f.lineWidth / this.scale : null});
                c = this.append(c, b, d)
            }
        }
    }
...
a.render = function() {
    ...
    function c(i, j, l) {
        ...
        if (k.visible) {
            ...
            for (q = 0; q < o; q++)
                i.children[q].scene = k.children[q];
            f[0] = k.data;
            if (n.scene)
            ...
            for (q = 0; q < o; q++)
                delete i.children[q].scene
        }
    }
    var d = this.parent, f = pv.Mark.stack;
    if (d && !this.root.scene)
        this.root.render();
    else {
        for (var g = [], h = this; h.parent; h = h.parent)
            g.unshift(h.childIndex);
        for (this.bind(); d && !d.hasOwnProperty("index"); )
            d = d.parent;
...
a.bind = function() {
    function b(j) {
        do
            for (var l = j.$properties, k = l.length - 1; k >= 0; k--) {
                var q = l[k];
                if (!(q.name in c)) {
                    c[q.name] = q;
                    switch (q.name) {
    ...
    do
        for (var i in h.properties)
            i in c || d[2].push(c[i] = {name: i,type: 2,value: null});
    while (h = h.proto);
    h = d[0].concat(d[1]);
    for (i = 0; i < h.length; i++)
        this.propertyMethod(h[i].name, 
        true);
...
a.build = function() {
    ...
    if (this.binds.defs.length) {
        ...
        for (var f = 0; f < this.binds.defs.length; f++) {
            ...
            if (!h || g.id > h.id)
                d[g.name] = {id: 0,value: g.type & 1 ? g.value.apply(this, c) : g.value}
        }
    }
    d = this.binds.data;
...
a.buildImplied = function(b) {
    ...
    if (h.height)
        b.height = j;
    if (h.textStyle && !b.textStyle)
        b.textStyle = 
        pv.Color.transparent;
    if (h.fillStyle && !b.fillStyle)
        b.fillStyle = pv.Color.transparent;
    if (h.strokeStyle && !b.strokeStyle)
        b.strokeStyle = pv.Color.transparent
};
a.mouse = function() {
    ...
    while (f = f.parent);
    for (; f = g.pop(); )
        d = d.translate(f.left(), f.top()).times(f.transform());
    d = d.invert();
...
a.context = function(b, c, d) {
    ...
    function g(k) {
        if (k) {
            ...
            var q;
            if (k.children)
                for (var o = 0, n = k.children.length; o < n; o++) {
                    q = k.children[o];
                    delete q.scene;
                    delete q.scale
                }
...
pv.Mark.dispatch = function(b, c, d) {
    ...
    if (!h)
        return g && pv.Mark.dispatch(b, g, c.parentIndex);
    ...
    if (this.index) {
        ...
        if (!d) {
            ...
            if (!this.scene[0].segmented)
                c.optional = c.optional.filter(f)
        }
        c = 0;
...
pv.Dot.prototype.buildImplied = function(b) {
    if (b.radius == null)
        b.radius = Math.sqrt(b.size);
    else if (b.size == null)
        b.size = b.radius * b.radius;
    pv.Mark.prototype.buildImplied.call(this, b)
};
pv.Label = function() {
    pv.Mark.call(this)
...
pv.Rule.prototype.buildImplied = function(b) {
    ...
    if (b.width != null || c == null && d == null || d != null && c != null)
        b.height = 0;
    else
        b.width = 0;
    pv.Mark.prototype.buildImplied.call(this, b)
...
a.buildImplied = function(b) {
    ...
    if (!b.transform)
        b.transform = pv.Transform.identity;
    pv.Mark.prototype.buildImplied.call(this, b)
};
pv.Image = function() {
    pv.Bar.call(this)
...
a.bind = function() {
    ...
    do
        b.image = c.$image;
    while (!b.image && (c = c.proto))
};
a.buildImplied = function(b) {
    pv.Bar.prototype.buildImplied.call(this, b);
    if (b.visible) {
        if (b.imageWidth == null)
            b.imageWidth = b.width;
        if (b.imageHeight == null)
            b.imageHeight = b.height;
...
pv.Wedge.prototype.buildImplied = function(b) {
    if (b.angle == null)
        b.angle = b.endAngle - b.startAngle;
    else if (b.endAngle == null)
        b.endAngle = b.startAngle + b.angle;
    pv.Mark.prototype.buildImplied.call(this, b)
};
...
a.step = function() {
    ...
    for (b = this.constraints; b; b = b.next)
        b.apply(this.particles, c);
    for (b = this.particles; b; b = b.next)
        b.fx = b.fy = 0;
    for (b = this.forces; b; b = b.next)
        b.apply(this.particles, c)
};
...
pv.Constraint.bound = function() {
    ...
    b.apply = function(f) {
        if (c)
            for (var g = f; g; g = g.next)
                g.x = g.x < c.min ? c.min : g.x > c.max ? c.max : g.x;
        if (d)
            for (g = f; g; g = g.next)
                g.y = g.y < d.min ? d.min : g.y > d.max ? d.max : g.y
    };
    return b
};
pv.Layout = function() {
...
pv.Layout.Network.prototype = pv.extend(pv.Layout).property("nodes", function(b) {
    ...
    return b.map(function(c) {
        if (isNaN(c.linkValue))
            c.linkValue = isNaN(c.value) ? 1 : c.value;
        return c
    })
});
...
pv.Layout.Network.prototype.buildProperties = function(b, c) {
    if ((b.$id || 0) < this.$id)
        pv.Layout.prototype.buildProperties.call(this, b, c)
};
...
pv.Layout.Hierarchy.prototype.buildImplied = function(b) {
    if (!b.links)
        b.links = pv.Layout.Hierarchy.links.call(this);
...
pv.Layout.Hierarchy.NodeLink = {buildImplied: function(b) {
        function c(m) {
...
pv.Layout.Hierarchy.Fill = {constructor: function() {
...
a.buildImplied = function(b) {
    ...
    function f(r, s) {
        ...
        if (m != "squarify")
            c(r.childNodes, r.size, m == "slice" ? true : m == "dice" ? false : s & 1, x, t, p, v);
        else {
            var w = [];
            s = Infinity;
...
pv.Layout.Pack.prototype.buildImplied = function(b) {
    function c(o) {
        ...
        for (var m = 0, r = o.length; m < r; m++) {
            var s = o[m];
            if (!s.firstChild)
                s.radius = i.$radius.apply(i, (n[0] = s, n))
        }
        n.shift()
    }
    function d(o) {
        ...
        if (o.length > 1) {
            ...
            if (o.length > 2) {
                ...
                for (var A = 3; A < o.length; A++) {
                    g(v, w, y = o[A]);
                    var D = 0, G = 1, E = 1;
                    for (z = w.n; z != w; z = z.n, G++)
                        if (s(z, y)) {
                            D = 1;
                            break
                        }
                    if (D == 1)
                        for (C = v.p; C != z.p; C = C.p, E++)
                            if (s(C, y)) {
                                if (E < G) {
                                    D = -1;
                                    z = C
...
pv.Layout.Force.prototype.buildImplied = function(b) {
    ...
    if (pv.Layout.Network.prototype.buildImplied.call(this, b)) {
        ...
        for (var d = this, f = b.nodes, g = b.links, h = b.iterations, i = b.width, j = b.height, l = 0, k; l < f.length; l++) {
            k = f[l];
            if (isNaN(k.x))
                k.x = i / 2 + 40 * Math.random() - 20;
            if (isNaN(k.y))
                k.y = j / 2 + 40 * Math.random() - 20
        }
        k = pv.simulation(f);
        ...
        if (h == null) {
            ...
            if (!this.$timer)
                this.$timer = setInterval(function() {
                    for (var q = false, o = d.binds.$force; o; o = o.next)
                        if (pv.max(o.nodes, c) > o.min) {
                            o.sim.step();
                            q = true
                        }
                    q && d.render()
...
pv.Layout.Cluster.prototype.buildImplied = function(b) {
    if (!pv.Layout.Hierarchy.prototype.buildImplied.call(this, b)) {
        ...
        c.visitAfter(function(l) {
            if (l.firstChild)
                l.depth = 1 + pv.max(l.childNodes, function(k) {
                    return k.depth
                });
            else {
                if (d && j != l.parentNode) {
        ...
        c.visitAfter(function(l) {
            if (l.firstChild)
                l.breadth = pv.mean(l.childNodes, function(k) {
                    return k.breadth
                });
            else {
                if (d && j != l.parentNode) {
...
pv.Behavior.point = function(b) {
    ...
    function d() {
        ...
        if (k.cost == Infinity || k.distance > l)
            k = null;
        if (g) {
            if (k && g.scene == k.scene && g.index == k.index)
                return;
...
pv.Behavior.pan = function() {
    function b() {
        ...
        j = 1 / (h.k * this.scale);
        if (l)
            l = {x: (1 - h.k) * this.width(),y: (1 - h.k) * this.height()}
    }
    function c() {
        if (f) {
            f.mark.context(f, g, function() {
...
pv.Geo.projections = {mercator: {project: function(b) {
            return {x: b.lng / 180,y: b.lat > 85 ? 1 : b.lat < -85 ? -1 : Math.log(Math.tan(Math.PI / 4 + pv.radians(b.lat) / 2)) / Math.PI}
        },invert: function(b) {
            return {lng: b.x * 180,lat: pv.degrees(2 * Math.atan(Math.exp(b.y * Math.PI)) - Math.PI / 2)}
        }},"gall-peters": {project: function(b) {
            return {x: b.lng / 180,y: Math.sin(pv.radians(b.lat))}
        },invert: function(b) {
            return {lng: b.x * 180,lat: pv.degrees(Math.asin(b.y))}
        }},sinusoidal: {project: function(b) {
            return {x: pv.radians(b.lng) * Math.cos(pv.radians(b.lat)) / Math.PI,
                y: b.lat / 90}
        },invert: function(b) {
            return {lng: pv.degrees(b.x * Math.PI / Math.cos(b.y * Math.PI / 2)),lat: b.y * 90}
        }},aitoff: {project: function(b) {
            var c = pv.radians(b.lng);
            b = pv.radians(b.lat);
            var d = Math.acos(Math.cos(b) * Math.cos(c / 2));
            return {x: 2 * (d ? Math.cos(b) * Math.sin(c / 2) * d / Math.sin(d) : 0) / Math.PI,y: 2 * (d ? Math.sin(b) * d / Math.sin(d) : 0) / Math.PI}
        },invert: function(b) {
            var c = b.y * Math.PI / 2;
            return {lng: pv.degrees(b.x * Math.PI / 2 / Math.cos(c)),lat: pv.degrees(c)}
        }},hammer: {project: function(b) {
            var c = pv.radians(b.lng);
            b = pv.radians(b.lat);
...
pv.Geo.scale = function(b) {
    ...
    var g = {x: 0,y: 0}, h = {x: 1,y: 1}, i = [], j = pv.Geo.projections.identity, l = pv.Scale.linear(-1, 1).range(0, 1), k = pv.Scale.linear(-1, 1).range(1, 0), q = {lng: 0,lat: 0}, o, n;
    c.x = function(m) {
        return c(m).x
    };
    c.y = function(m) {
